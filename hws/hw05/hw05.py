...
def make_counter():
    ...
    appeared_string = {}
    def counter(string):
        nonlocal appeared_string
        if string in appeared_string:
            appeared_string[string] += 1
        else:
            appeared_string[string] = 1
        return appeared_string[string]
    return counter
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock <= 0:
            ...
            add_amount = self.price - self.balance
            return 'You must deposit ${} more.'.format(add_amount)
        else:
