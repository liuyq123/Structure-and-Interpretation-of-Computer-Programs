...
def balanced(m):
    ...
    left_mobile = left(m)
    left_torque = 1
    while is_side(left_mobile):
        left_torque *= length(left_mobile)
        left_mobile = end(left_mobile)
        if is_weight(left_mobile):
            left_mobile = size(left_mobile)
    left_torque *= left_mobile

    right_mobile = right(m)
    right_torque = 1
    while is_side(right_mobile):
        right_torque *= length(right_mobile)
        right_mobile = end(right_mobile)
        if is_weight(right_mobile):
            right_mobile = size(right_mobile)
    right_torque *= right_mobile
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    def try_withdraw(amount, attempt):
